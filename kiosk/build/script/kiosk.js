  ...
  if (this.readyState == 4 && this.status == 200) {
    ...
    for (let i = 0; i < menu_list_arr.length; i++) {
      ...
      for (let j = 0; j < price_arr.length; j++) {
        if (price_arr[j][0] == menu_list_arr[i].name) {
          price_arr.splice(j, 1);
          break;
        }
      }
        for (let i = 0; i < price_arr.length; i++) {
        ...
        if (price_arr.length == 0) {
          $('.sum_value').html('0원');
          $('.pay_list_sum').html('0원');
          $('.receipt_list_price').html('0원');
        }
        price_counter = 0;
      });
...
$('.extra_order_next_box').click(function() {
  ...
  /*키오스크 메뉴 셀렉트 드링크 매장,포장,hot,ice 선택영역*/
    ...
    let hotice_selected = !($('.hotice_box').css('background-color') === 'rgb(85, 85, 85)' && 
                            $('.hotice_box2').css('background-color') === 'rgb(85, 85, 85)');
    
    if (!store_selected && !hotice_selected) {
      $('.model_font').html('매장/포장 및 HOT/ICE를 선택하세요.');
      $('.model_box').show();
      return;
    } else if (!store_selected) {
      $('.model_font').html('매장 혹은 포장을 선택하세요.');
      $('.model_box').show();
      return;
    } else if (!hotice_selected) {
...
$('.pointuse_button').click(function() {
  ...
  let remaining_points = parseInt(remaining_text);
  let use_points = parseInt(val);

  if (val == "" || isNaN(use_points) || use_points <= 0) {
...
$('.pointsave_button').click( () => {
  let val = $('.point_save_input_box > input').val();
  if (val == "" || val.length < 11) {
    $('.model_font').html('번호를 11자리 입력하세요.');
    $('.model_box').show();
    return;
    ...
    switch(index){
      ...
      default:
        if($('.number_input_box > input').val().length < 11){
          let val = $(this).find('p').html();
          if (val !== undefined) {
            $('.number_input_box > input')[0].value += val;
          }
        }
        break;
    };
    ...
    switch(index){
      ...
      default:
        if($('.number_inquire_input_box > input').val().length < 11){
          let val = $(this).find('p').html();
          if (val !== undefined) {
            $('.number_inquire_input_box > input')[0].value += val;
          }
        }
      break;
    };
    ...
    switch(index){
      ...
      default:
        if($('.point_remaining_input_box > input').val().length < 3){
          let val = $(this).find('p').html();
          if (val !== undefined) {
            $('.point_remaining_input_box > input')[0].value += val;
          }
        }
      break;
    };
    ...
    switch(index){
      ...
      default:
        if($('.point_save_input_box > input').val().length < 11){
          let val = $(this).find('p').html();
          if (val !== undefined) {
            $('.point_save_input_box > input')[0].value += val;
          }
        }
      break;
    };
