...
# 프로젝트 개요 : 
개인 포트폴리오 블로그 입니다. 개발자로서의 정체성, 보유 기술 스택, 그리고 그동안 진행해온 주요 프로젝트들을 한눈에 확일할 수 있도록 인터렉티브하게 설계되었습니다.

# 스킬 스택 :
Frontend: HTML5 (Pug), CSS3 (SCSS), JavaScript (jQuery, TypeScript)
...
# 프로젝트 구조 :
src/pug: 사이트 구조 설계 (blog.pug)
src/scss: 디자인 스타일링 (blog.scss)
src/script: 인터랙션 로직 (blog.ts)
public/img: 포트폴리오 및 스킬 아이콘 이미지
build/: 최종 배포용 컴파일 파일
