...
- **개별 프로젝트 README**: 프로젝트 특성 분석 후 7대 필수 항목(프로젝트 이름 / 프로젝트 소개 (서비스의 핵심 가치와 한 줄 요약 중심) / 프로젝트 개요 (제작 배경 / 기획 의도 / 프로젝트 목표 중심) / 주요 기능 (핵심 기능을 사용자 관점에서 정리) / 기술스택 / 기술 선택 이유 / 시스템 아키텍처 (서버 / DB / 외부 API / 등의 관계를 다이어그램으로 표현) / 폴더 구조 (디렉터리 다이어그램으로 표현) / 트러블슈팅 (문제 / 원인 / 해결 방법 / 결과) / 성능 개선 (개선 전/후 수치와 측정 방법) / 실행 방법 (환경 설정 / 설치 / 실행 명령어))을 반영하여 작성합니다.
